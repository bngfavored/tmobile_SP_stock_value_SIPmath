        ...
        if term is None:
            ...
            for i in terms_for_loop:
                ...
                if m['Validation']['valid'][i] == 'yes':
                    ...
                    # # ax[j + 1].axes.xaxis.set_ticks([])
                    # ax[j + 1].axes.yaxis.set_ticks([])
                    # ax[j+1].axis([min(res_data['quantileValues']),
                    #              max(res_data['quantileValues']), 0, 1])
                    # ax[j].axes.xaxis.set_ticks([])
                    ...
                    #     ax[0].set(title=chart_title, ylabel='PDF',
                    #               xlabel='Quantiles')
...
#     'Input Data Type', ('CSV File', 'Quantile', 'API'), index=0)
...
#         input_df = None
#         uploaded_file = uploaded_file if isinstance(
#             uploaded_file, list) else [uploaded_file]
...
#             table_container.write(input_df[:10].to_html(
#                 index=False), unsafe_allow_html=True)
#         if isinstance(input_df, pd.DataFrame):
#             # if "column_index" not in st.session_state:
#             st.session_state["column_index"] = {
#                 k: v for v, k in enumerate(input_df.columns)}
...
#             selected_column = graphs_container.selectbox(
#                 "Select Current Variable:",  input_df.columns, key="Big Graph Column")
...
#             if boundedness == "'b' - bounded on both sides":
#                 # convert to float and list
#                 boundsl = seed_container.number_input('Lower Bound', max_value=input_df[selected_column].min(
#                 ), value=input_df[selected_column].min(), format="%f", key=f"Column_lower")
#                 # if
#                 # seed_container.number_input('Lower Bound')
#                 boundsu = seed_container.number_input('Upper Bound', max_value=input_df[selected_column].max(
#                 ), value=input_df[selected_column].max(), key=f"Column_upper")
...
#                 if 'mfitted' in st.session_state and selected_column in st.session_state['mfitted'][data_type_str]:
#                     print("selected_column", selected_column)
#                     st.session_state["Column_Terms"] = st.session_state['mfitted'][data_type_str][selected_column]['options']['terms'] if st.session_state[
#                         'mfitted'][data_type_str][selected_column]['options']['terms'] else "--------Enter number of terms--------"

#             boundedness = boundedness.strip().split(" - ")[0].replace("'", "")
#             terms_list = [
#                 "--------Enter number of terms--------", *list(range(3, 17))]
#             # col_terms = None
...
#                         st.session_state['mfitted'][data_type_str][col]['options']['seeds']['arguments']["varId"] = str(float(
#                             st.session_state['mfitted'][data_type_str][col]['options']['seeds']['arguments']["varId"]) + st.session_state["column_index"][col])
...
#                                                   user_terms=col_terms,
#                                                   graphs=False)
#             previous_button = left_buttons.button(
#                 "Previous Variable", on_click=update_counter, args=(-1,), key=f"previous_button")
#             next_button = right_buttons.button(
#                 "Next Variable", on_click=update_counter, args=(1,), key=f"next_button")
...
#                 st.session_state['mfitted'][data_type_str][selected_column]['options']['seeds']['arguments']["varId"] = str(float(
#                     st.session_state['mfitted'][data_type_str][default_column]['options']['seeds']['arguments']["varId"]) + st.session_state["column_index"][selected_column])

#             if 'mfitted' in st.session_state and selected_column in st.session_state['mfitted'][data_type_str]:
#                 st.session_state[f"entity {selected_column}"] = str(
#                     st.session_state['mfitted'][data_type_str][selected_column]['options']['seeds']['arguments']["entity"])
#                 st.session_state[f"varId {selected_column}"] = str(
#                     st.session_state['mfitted'][data_type_str][selected_column]['options']['seeds']['arguments']["varId"])
#                 st.session_state[f"seed3 {selected_column}"] = str(
#                     st.session_state['mfitted'][data_type_str][selected_column]['options']['seeds']['arguments']["seed3"])
#                 st.session_state[f"seed4 {selected_column}"] = str(
#                     st.session_state['mfitted'][data_type_str][selected_column]['options']['seeds']['arguments']["seed4"])
#             seed_data = [left_values.text_input(f'entity', value="0", key=f"entity {selected_column}"),
#                          right_values.text_input(
#                              f'varId', value="1", key=f"varId {selected_column}"),
#                          left_values.text_input(
#                              f'seed 3', value="0",  key=f"seed3 {selected_column}"),
...
#             st.session_state['quantiles_data'][current_variable_name] = {
#                 'pos': num}
...
#                 st.session_state[f"Column_Terms {quantile_name} {quantile_index}"] = st.session_state[
#                     'quantiles_data'][quantile_name]['col_terms']
...
#                     st.session_state[f"entity {quantile_name}"] = st.session_state[
#                         'quantiles_data'][quantile_name]['seeds']['arguments']['entity']
#                     st.session_state[f"varId {quantile_name}"] = st.session_state[
#                         'quantiles_data'][quantile_name]['seeds']['arguments']['varId']
#                     st.session_state[f"seed3 {quantile_name}"] = st.session_state[
#                         'quantiles_data'][quantile_name]['seeds']['arguments']['seed3']
#                     st.session_state[f"seed4 {quantile_name}"] = st.session_state[
#                         'quantiles_data'][quantile_name]['seeds']['arguments']['seed4']
...
#                                                             args=(
#                                                                 quantile_name, f"y values {quantile_name} {num*i}", i - 1),
...
#                                                             on_change=update_values,
#                                                             args=(
#                                                                 quantile_name, f"x values {quantile_name} {num*i}", i - 1),
...
#                                  on_change=update_terms,
#                                  args=(quantile_name, data_type_str,
#                                        quantile_index),
...
#                 st.session_state["Column_lower"] = st.session_state["Column_lower"] if st.session_state["Column_lower"] <= q_data[quantile_name].min(
#                 ) else q_data[quantile_name].min()
...
#                 st.session_state["Column_upper"] = st.session_state["Column_upper"] if st.session_state["Column_upper"] >= q_data[quantile_name].max(
#                 ) else q_data[quantile_name].max()
...
#                                             kwargs=dict(
#                                                 data_type=data_type_str, variable_name=quantile_name),
...
#                           'seed3':  seed_data[2],
#                           'seed4':  seed_data[3]}}
#         previous_button = left_values.button(
#             "Previous Variable", on_click=update_counter, args=(-1,), key=f"previous_button")
#         next_button = right_values.button(
#             "Next Variable", on_click=update_counter, args=(1,), key=f"next_button")
#         make_graphs_button = left_values.button(
#             "Make Graph Panel", key=f"make_graph_panel_button{num}")
...
#     pd_data = pd.concat([st.session_state['quantiles_data'][x[1]]['q_data'] for x in enumerate(
#         st.session_state['quantiles_data']) if x[0] < quantile_number_variable and 'q_data' in st.session_state['quantiles_data'][x[1]]])
#     print(pd_data)
#     if isinstance(pd_data, pd.DataFrame):
#         st.session_state["column_index"] = {
#             k: v for v, k in enumerate(pd_data.columns)}
...
#     st.session_state["Big Graph Column"] = st.session_state[
#         f"Quantile Name {st.session_state['quantile_counter']}"]
...
#     selected_column = graphs_container.selectbox(
#         "Select Current Variable:", variable_list, on_change=update_input_name, key="Big Graph Column")
#     # upper_bound, lower_bound = seed_container.columns(2)
#     # TODO: error handling for value too small for upper bounds or letter
...
#         if big_plots:
#             col_terms = int(st.session_state['quantiles_data'][selected_column]['col_terms']) if isinstance(
#                 st.session_state['quantiles_data'][selected_column]['col_terms'], int) else None
...
#                 print(
#                     pd_data.loc[pd_data[selected_column].notnull(), [selected_column]])
#                 print(pd_data[[selected_column]])
#                 pd_data.loc[pd_data[selected_column].notnull(), [selected_column]].apply(make_csv_graph,
#                                                                                          probs=pd_data[pd_data[selected_column].notnull(
#                                                                                          )].index.to_list(),
#                                                                                          boundedness=st.session_state['quantiles_data'][
#                                                                                              selected_column]['boundedness'],
#                                                                                          bounds=st.session_state['quantiles_data'][
#                                                                                              selected_column]['bounds'],
#                                                                                          big_plots=big_plots,
#                                                                                          user_terms=col_terms,
#                                                                                          graphs=st.session_state['quantiles_data'][selected_column]['make_graphs_button'])
#                 st.session_state['mfitted'][data_type_str][selected_column]['options'][
#                     'seeds'] = st.session_state['quantiles_data'][selected_column]['seeds']
#                 # if 'mfitted' in st.session_state and selected_column in st.session_state['mfitted'][data_type_str] and 'seeds' not in st.session_state['mfitted'][data_type_str][selected_column]['options']:
#                 # st.session_state['mfitted'][data_type_str][selected_column]['options']['seeds'] = {
#                 # 'name':'hdr'+str(st.session_state["column_index"][selected_column]+1),
#                 # 'function':'HDR_2_0',
